...
func (r *UserRepo) GetByID(id string) (*domain.User, error) {
	query := `SELECT id, email, password_hash, full_name, phone, avatar_url, is_active, created_at, updated_at FROM users WHERE id = $1`
	row := r.db.QueryRow(context.Background(), query, id)
	return scanUser(row)
}

func (r *UserRepo) GetByEmail(email string) (*domain.User, error) {
	query := `SELECT id, email, password_hash, full_name, phone, avatar_url, is_active, created_at, updated_at FROM users WHERE email = $1`
...
func (r *UserRepo) Update(user *domain.User) error {
	query := `
		UPDATE users SET email=$2, full_name=$3, phone=$4, avatar_url=$5, is_active=$6, updated_at=NOW()
		WHERE id=$1
	`
	_, err := r.db.Exec(context.Background(), query,
...
func scanUser(scanner pgx.Row) (*domain.User, error) {
	...
	err := scanner.Scan(
		...
		&u.Phone, &u.AvatarURL, &u.IsActive, &u.CreatedAt, &u.UpdatedAt,
	)
	if err != nil {
